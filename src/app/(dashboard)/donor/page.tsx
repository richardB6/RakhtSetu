...
export default function DonorDashboardPage() {
  ...
  const toggleNotifications = async () => {
    if (!data) return;
    const enabled = !data.availability.emergencyNotificationsEnabled;
    setMessage(null);
    try {
      const response = await fetch('/api/donor/availability', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ emergencyNotificationsEnabled: enabled }),
      });
      const payload = await response.json();
      if (!response.ok || !payload.success) throw new Error(payload.message || 'Unable to update notification preferences');
      setData((current) => current ? {
        ...current,
        availability: { ...current.availability, emergencyNotificationsEnabled: enabled },
      } : current);
      setMessage(`Emergency notifications ${enabled ? 'enabled' : 'disabled'}.`);
    } catch (reason) {
      setMessage(reason instanceof Error ? reason.message : 'Unable to update notification preferences');
    }
  };
